...
// 0.001 ETH = 100,000 BKT  →  1 ETH = 100,000,000 BKT
...
export default function ExchangePage() {
  ...
  const ethWei = (() => { try { return ethInput ? parseEther(ethInput) : 0n; } catch { return 0n; } })();
  const bktRaw = (() => { try { return bktInput ? parseUnits(bktInput, 18) : 0n; } catch { return 0n; } })();
  ...
  return (
    <div className="max-w-md mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">ETH를 BKT로 환전하여 마켓플레이스 거래에 사용하세요</p>
      </div>
        ...
        <div className="text-right">
          ...
          {priceLoading ? (
            ...
            <>
              ...
              <p className="text-xs text-gray-400">1 ETH / USD · 실시간</p>
            </>
          ) : (
            <p className="text-sm text-gray-400">시세 불러오기 실패</p>
          )}
        </div>
      </div>

      {!isDeployed && (
        <div className="rounded-xl px-4 py-3 text-sm mb-4" style={{ background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.2)", color: "#fbbf24" }}>
          환전 컨트랙트가 아직 배포되지 않았습니다.<br />
          <span className="text-xs">BlockToken + Exchange 배포 후 .env.local을 업데이트해주세요.</span>
        </div>
      )}
      ...
      <div className="card p-6">
        ...
        <div className="flex gap-1 mb-6 rounded-xl p-1" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <button
            ...
            onClick={() => { setTab("sell"); setTxError(""); }}
          >
            BKT → ETH
          </button>
        </div>
        ...
        {tab === "buy" && (
          <div className="space-y-4">
            ...
            {isSuccess && <p className="text-xs text-emerald-600">✓ 환전 완료!</p>}
            <button
              className="btn-primary w-full"
              disabled={!ethWei || isLoading || !isDeployed}
              onClick={handleBuy}
            >
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="100000"
                value={bktInput}
                onChange={(e) => setBktInput(e.target.value)}
              />
              {bktBalance !== undefined && (
                <button
                  className="text-xs text-brand-500 mt-1 hover:underline"
                  onClick={() => setBktInput(formatUnits(bktBalance as bigint, 18))}
                >
            ...
            {txError && <p className="text-xs text-red-500">{txError}</p>}
            {isSuccess && <p className="text-xs text-emerald-600">✓ 완료!</p>}
            {needsApprove ? (
              <button
                className="btn-primary w-full"
                disabled={!bktRaw || isLoading || !isDeployed}
      ...
      <div className="mt-4 rounded-xl bg-gray-50 border border-gray-100 px-4 py-3 text-xs text-gray-500 leading-relaxed">
        <strong className="text-gray-700">BKT (Block Token)</strong><br />
        블록마켓 전용 토큰으로 경매·구매·판매에 사용됩니다.<br />
        환율: 0.001 ETH = 100,000 BKT (1 ETH = 1억 BKT)
      </div>
    </div>
  );
}
