...
import { useChatRooms, useChatMessages, useSendMessage, useMarkRead, ChatRoom } from "@/hooks/useChat";
import { useReadContracts } from "wagmi";
import MarketplaceABI from "@/abi/Marketplace.json";
import { getContracts } from "@/config/contracts";
import { useChainId } from "wagmi";
import { truncateAddress, formatBKT } from "@/lib/utils";
...
function MessageView({ room, myAddr }: { room: ChatRoom; myAddr: string }) {
  ...
  const handleSend = async () => {
    ...
    if (input.trim().length > MAX_MSG_LEN) {
      setSendError(`메시지는 ${MAX_MSG_LEN}자 이하로 입력해주세요.`);
      return;
    }
    setSendError("");
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-t border-gray-100 bg-white space-y-1.5">
        ...
        {input.length > MAX_MSG_LEN * 0.8 && (
          <p className="text-[10px] text-gray-400 text-right">{input.length}/{MAX_MSG_LEN}</p>
        )}
      </div>
    </div>
  );
...
function ChatPageInner() {
  ...
  const [showList, setShowList] = useState(true); // 모바일: 목록↔메시지 전환

  const filteredRooms = filterListingId
    ...
    return (
      <div className="max-w-xl mx-auto pt-10 text-center">
        <div className="card p-10">
          ...
          <p className="text-sm text-gray-500 mb-4">채팅을 이용하려면 로그인이 필요해요.</p>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="card overflow-hidden flex" style={{ height: "calc(100vh - 160px)", minHeight: 480 }}>
        {/* 채팅방 목록 — 모바일에서 showList일 때만 표시 */}
        <div className={`flex-shrink-0 border-r border-gray-100 flex flex-col
          w-full sm:w-72 ${showList ? "flex" : "hidden sm:flex"}`}>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
            {loading ? (
              ...
              <div className="py-12 text-center px-4">
                <p className="text-sm text-gray-400">아직 채팅이 없어요.</p>
              </div>
            ) : (
              filteredRooms.map((room) => (
                <RoomItem
                  ...
                  onClick={() => { setSelectedRoom(room); setShowList(false); refetch(); }}
          ...
          {selectedRoom ? (
            <>
              ...
              <button
                ...
                className="sm:hidden flex items-center gap-1 px-4 py-2 text-xs text-brand-500 border-b border-gray-100"
              >
                ← 목록으로
              </button>
