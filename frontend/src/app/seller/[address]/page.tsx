...
function StarRating({ avg, total }: { avg: number | null; total: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex">
        {[1, 2, 3, 4, 5].map((s) => (
          <span key={s} className={s <= Math.round(avg) ? "text-amber-400" : "text-gray-200"}>
            ★
          </span>
        ))}
      </div>
...
export default function SellerPage({ params }: { params: { address: string } }) {
  ...
  const sellerListings = useMemo(
    () => ((allListings as Listing[] | undefined) ?? []).filter((l) => l.seller.toLowerCase() === sellerAddr.toLowerCase()),
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <Link href="/marketplace" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-gray-600 mb-5 transition">
        ← 목록
      </Link>
      ...
      <div className="card p-6 mb-6 flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <a href={`https://sepolia.etherscan.io/address/${sellerAddr}`} target="_blank" rel="noreferrer" className="text-xs text-brand-500 hover:underline">
            Etherscan에서 보기 →
          </a>
          <div className="mt-2">
              ...
              <div key={r._id} className="border-b border-gray-50 pb-3 last:border-0 last:pb-0">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <span key={s} className={`text-sm ${s <= r.rating ? "text-amber-400" : "text-gray-200"}`}>
                        ★
                      </span>
                    ))}
                  </div>
                ...
                <p className="text-xs text-gray-500">
                  {truncateAddress(r.reviewer)} · <span className="text-gray-400">{r.role === "buyer" ? "구매자" : "판매자"}</span>
                </p>
      ...
      {isLoading ? (
        ...
        <ItemGrid listings={sellerListings} stateMap={stateMap} endTimeMap={endTimeMap} viewCounts={viewCounts} emptyMessage="등록된 상품이 없습니다." />
      )}
    </div>
  );
}
