...
const FEATURES = [
  {
    icon: "🛒",
    title: "즉시구매",
    desc: "원하는 가격에 바로 거래하고, 스마트 컨트랙트가 결제와 정산을 안전하게 처리합니다.",
  ...
  },
  {
    icon: "🏷️",
    title: "공개경매",
    desc: "실시간으로 최고 입찰가가 갱신되는 영어식 경매를 지원합니다.",
  ...
  },
  {
    icon: "🕶️",
    title: "블라인드경매",
    desc: "입찰가를 숨긴 뒤 공개 단계에서 검증하는 Vickrey 경매를 사용할 수 있습니다.",
    ...
    <div className="relative overflow-hidden">
      ...
      <div className="relative text-center pt-16 pb-20">
        ...
        <p className="text-base sm:text-lg leading-relaxed mb-10 max-w-lg mx-auto" style={{ color: "#7878a0" }}>
          ...
          스마트 컨트랙트가 구매부터 정산까지 자동으로 처리합니다.
        </p>

        <div className="flex items-center justify-center gap-4">
          <Link href="/marketplace" className="btn-primary text-sm px-7 py-3">
            거래 목록 보기
          </Link>
          <Link href="/sell" className="btn-secondary text-sm px-7 py-3">
            상품 등록하기
          </Link>
        </div>
      ...
      <div className="max-w-5xl mx-auto w-full grid grid-cols-1 sm:grid-cols-3 gap-4 pb-12">
        {FEATURES.map((f) => (
          ...
          >
            ...
            <h3 className="font-bold text-sm mb-2" style={{ color: "#f0f0f8" }}>
              {f.title}
            </h3>
      ...
      >
        <span className="text-brand-400 mt-0.5 text-base flex-shrink-0">ℹ️</span>
        <p className="text-xs leading-relaxed" style={{ color: "#7878a0" }}>
