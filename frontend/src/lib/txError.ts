export function parseTxError(e: unknown): string {
  ...
  if (msg.includes("Already committed"))
    return "이미 입찰했습니다.";
  if (msg.includes("Token transfer failed") || msg.includes("ERC20InsufficientAllowance") || msg.includes("ERC20InsufficientBalance"))
    return "BKT 토큰 전송에 실패했습니다. 잔액과 승인 상태를 확인해주세요.";
  if (msg.includes("Deposit below reserve"))
    return "보증금이 최소 입찰가 미만입니다.";
  if (msg.includes("Commit phase ended"))
    return "입찰 커밋 단계가 종료되었습니다.";
